 ...
 * do pagamento (a janela de cancelamento; 0 = na hora; `null` = volta ao
 * padrão). Vale também pros pedidos que já estão esperando: a janela conta do
 * pagamento, a cada vez.
 */
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (
    ...
    res.status(400).json({
      message: `minutos é um número inteiro de 0 a ${JANELA_MAXIMA_DA_NOTA_MIN} (ou null, o padrão)`,
    })
    return
  }
  const erp = erpDaTela()
